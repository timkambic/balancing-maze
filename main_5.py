...
class CONTROLL_MAIN():
	...
	def Initialize(self,maze_x,maze_y,low_c,high_c): #input from changeable-variables
		...
		self.serial.write("15,16,"+str(self.brightness_led)+",") #level out maze
		...
		self.pid_x = PID_controller()
		self.pid_y = PID_controller()
		Kp = 0.035
		Ki  = 0
		Kd = 0	
		self.pid_x.Initialize(Kp,Ki,Kd)
		self.pid_y.Initialize(Kp,Ki,Kd)
	...
	def Init_display(self):
		...
		cv2.moveWindow('cpy', 400,0)
		cv2.moveWindow('solved',900,0)
	...
	def DoStuff(self):
		...
		output_x = self.pid_x.Compute(delta_x) +15
		output_y = self.pid_y.Compute(delta_y) +15
		output_x= int(round(output_x,0))
		output_y = int(round(output_y,0))*-1 +30 # invert y axis
